...
// Carga inicial de la lista según selección
        ...
        findViewById(R.id.btnIngresoMensual).setOnClickListener(v -> {
            View dialogView = getLayoutInflater().inflate(R.layout.dialog_ingreso_mensual, null);
            EditText input = dialogView.findViewById(R.id.edtIngresoMensualDialog);
            Button btnGuardar = dialogView.findViewById(R.id.btnGuardarIngresoMensual);
            Button btnCancelar = dialogView.findViewById(R.id.btnCancelarIngresoMensual);

            float ingresoActual = prefs.getFloat("ingreso_mensual", 0f);
            if (ingresoActual > 0f) {
                input.setText(String.format(Locale.US, "%.2f", ingresoActual));
            ...
            btnCancelar.setOnClickListener(cancelar -> dialog.dismiss());
            btnGuardar.setOnClickListener(guardar -> {
                try {
                    ...
                    float ingreso = Float.parseFloat(valor);
                    prefs.edit().putFloat("ingreso_mensual", ingreso).apply();
                    actualizarResumen();
                    dialog.dismiss();
                } catch (NumberFormatException e) {
                    input.setError("Monto invalido");
                }
            });
